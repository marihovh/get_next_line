...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/17 10:37:00 by marihovh          #+#    #+#             */
/*   Updated: 2023/02/17 10:38:56 by marihovh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*ret_line;
	static char	*stat_line[4096];

	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	stat_line[fd] = reading(fd, stat_line[fd]);
	if (!stat_line[fd])
		return (0);
	ret_line = ret(stat_line[fd]);
	stat_line[fd] = save(stat_line[fd]);
	return (ret_line);
}
